...
- (CGPoint)configueCurrentPoint:(CGPoint)point
{
    //如果小于初始值的话 直接置为初始值
    if (point.x<0) {
        point.x = 0;
    ...
    if (point.x > _selRect.size.width) {
        point.x = _selRect.size.width;
    }
    
    if (point.y > _selRect.size.height) {
        point.y = _selRect.size.height;
    }
...
- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    if ([event allTouches].count == 1) {
        ...
        BHBPaintPath *path = [BHBPaintPath paintPathWithLineWidth:_width
                                                     startPoint:[self configueCurrentPoint:startP]];
        _path = path;
...
- (void)touchesMoved:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    if ([event allTouches].count > 1){
        ...
        [_path addLineToPoint:[self configueCurrentPoint:moveP]];
